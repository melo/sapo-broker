...
SAPO_BROKER_T *sb_new(char *hostname, int port, int type)
{
    ...
    if (conn == NULL) {
        _sb_set_error
            ("[SB]Out of memory!Cannot allocate memory for SAPO_BROKER_T");
        return NULL;
    }
...
int sb_connect(SAPO_BROKER_T * conn)
{
    ...
    if ((connect
         (conn->socket, (struct sockaddr *) &server_addr,
...
static int _sb_write(int socket, char *msg, int size)
{
    ...
    do {
        ...
        if (err == ENOTCONN) {
            _sb_set_errorf("[SB]Error writing (Not connected):",
                           strerror(errno));
                    ...
                    int timetolive)
{
    ...
    if (type == EQUEUE_QUEUE) {
        ...
        _sb_set_errorf
            ("[SB]Publish:Unknown message type %d. Not sending message.", type);
    ...
    // fill in the blanks
    if (timetolive) {
        sprintf(body, TPUBLISH_TIME, msg_type, ttl_str, topic, payload,
                msg_type);
    } else {
...
BrokerMessage *sb_receive(SAPO_BROKER_T * conn)
{
    ...
    if (_sb_read(conn->socket, (char *) &nbody_len, sizeof(uint32_t)) <= 0) {
        _sb_set_errorf("[SB]Error reading size of the message: %s",
                       strerror(errno));
        sb_disconnect(conn);
        return 0;
    }
    ...
    if (_sb_read(conn->socket, body, body_len) < 0) {
        _sb_set_errorf("[SB]Error reading the body of the message:%s",
                       strerror(errno));
        if (allocated) {
            free(body);
            body = NULL;
